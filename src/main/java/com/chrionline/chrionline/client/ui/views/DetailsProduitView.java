...
public class DetailsProduitView extends BorderPane {
    ...
    private VBox buildContent() {
        ...
        retourBtn.setStyle("-fx-background-color: transparent; -fx-text-fill: #7F5539; -fx-font-size: 13px; -fx-cursor: hand; -fx-padding: 0;");
        retourBtn.setOnMouseEntered(e -> retourBtn.setStyle("-fx-background-color: transparent; -fx-text-fill: #3B1F0E; -fx-font-size: 13px; -fx-cursor: hand; -fx-underline: true; -fx-padding: 0;"));
        retourBtn.setOnMouseExited(e -> retourBtn.setStyle("-fx-background-color: transparent; -fx-text-fill: #7F5539; -fx-font-size: 13px; -fx-cursor: hand; -fx-padding: 0;"));
    ...
    private StackPane buildImageSection() {
        ...
        container.setPrefSize(480, 360);
        container.setMinSize(480, 360);
        container.setMaxSize(480, 360);
        container.setStyle("-fx-background-color: #DDB892; -fx-background-radius: 16;");
    ...
    private VBox buildInfoSection() {
        ...
        Label categorieLabel = new Label(produit.getNomCategorie() != null ? produit.getNomCategorie() : "");
        categorieLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #B08968;");

        Label nomLabel = new Label(produit.getNom());
        nomLabel.setStyle("-fx-font-size: 32px; -fx-font-weight: bold; -fx-text-fill: #7F5539; -fx-wrap-text: true;");
        ...
        prixLabel.setStyle("-fx-font-size: 36px; -fx-font-weight: bold; -fx-text-fill: #7F5539;");
        ...
        Label stockLabel = new Label(enStock ? "En stock (" + produit.getStock() + " unités)" : "Rupture de stock");
        stockLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: " + (enStock ? "#16a34a" : "#dc2626") + ";");
        stockBox.getChildren().addAll(dot, stockLabel);
        prixStock.getChildren().addAll(prixLabel, stockBox);

        Label descLabel = new Label(produit.getDescription() != null ? produit.getDescription() : "");
        descLabel.setStyle("-fx-font-size: 14px; -fx-text-fill: #3B1F0E; -fx-wrap-text: true; -fx-line-spacing: 4;");
        descLabel.setMaxWidth(Double.MAX_VALUE);

        Label quantiteTitre = new Label("Quantité");
        quantiteTitre.setStyle("-fx-font-size: 14px; -fx-font-weight: bold; -fx-text-fill: #3B1F0E;");
        ...
        quantiteControl.setStyle("-fx-border-color: #DDB892; -fx-border-radius: 8; -fx-border-width: 2; -fx-background-radius: 8;");

        Button moinsBtn = createQuantiteBtn("-");
        moinsBtn.setOnAction(e -> { if (quantite > 1) { quantite--; quantiteLabel.setText(String.valueOf(quantite)); updateTotal(); } });

        quantiteLabel = new Label("1");
        quantiteLabel.setStyle("-fx-font-size: 18px; -fx-text-fill: #3B1F0E; -fx-padding: 8 20 8 20; -fx-font-weight: bold;");

        Button plusBtn = createQuantiteBtn("+");
        plusBtn.setOnAction(e -> { if (quantite < produit.getStock()) { quantite++; quantiteLabel.setText(String.valueOf(quantite)); updateTotal(); } });
        ...
        totalLabel.setStyle("-fx-font-size: 22px; -fx-font-weight: bold; -fx-text-fill: #7F5539;");
        ...
        panierBtn.setStyle("-fx-background-color: #7F5539; -fx-text-fill: #EDE0D4; -fx-font-size: 16px; -fx-font-weight: bold; -fx-background-radius: 10; -fx-padding: 14 24 14 24; -fx-cursor: hand;");
        panierBtn.setOnMouseEntered(e -> panierBtn.setStyle("-fx-background-color: #6A4730; -fx-text-fill: #EDE0D4; -fx-font-size: 16px; -fx-font-weight: bold; -fx-background-radius: 10; -fx-padding: 14 24 14 24; -fx-cursor: hand;"));
        panierBtn.setOnMouseExited(e -> panierBtn.setStyle("-fx-background-color: #7F5539; -fx-text-fill: #EDE0D4; -fx-font-size: 16px; -fx-font-weight: bold; -fx-background-radius: 10; -fx-padding: 14 24 14 24; -fx-cursor: hand;"));
        panierBtn.setOnAction(e -> ajouterAuPanier());

        info.getChildren().addAll(categorieLabel, nomLabel, prixStock, descLabel, new Separator(), quantiteTitre, quantiteRow, panierBtn);
        return info;
    }
    ...
    private Button createQuantiteBtn(String text) {
        ...
        btn.setStyle("-fx-background-color: transparent; -fx-text-fill: #7F5539; -fx-font-size: 18px; -fx-font-weight: bold; -fx-cursor: hand; -fx-border-color: transparent;");
        btn.setOnMouseEntered(e -> btn.setStyle("-fx-background-color: #E6CCB2; -fx-text-fill: #7F5539; -fx-font-size: 18px; -fx-font-weight: bold; -fx-cursor: hand; -fx-border-color: transparent;"));
        btn.setOnMouseExited(e -> btn.setStyle("-fx-background-color: transparent; -fx-text-fill: #7F5539; -fx-font-size: 18px; -fx-font-weight: bold; -fx-cursor: hand; -fx-border-color: transparent;"));
        return btn;
    }

    private VBox buildTabsSection() {
        VBox section = new VBox(16);
        section.setStyle("-fx-background-color: #EDE0D4; -fx-background-radius: 16; -fx-effect: dropshadow(gaussian, rgba(127,85,57,0.08), 12, 0, 0, 2);");
        ...
        descTab.setOnAction(e -> { styleTabActive(descTab); styleTabInactive(caracTab); showDescriptionTab(tabContent); });
        caracTab.setOnAction(e -> { styleTabActive(caracTab); styleTabInactive(descTab); showCaracteristiquesTab(tabContent); });
    ...
    private void styleTabActive(Button btn) {
        btn.setStyle("-fx-background-color: #7F5539; -fx-text-fill: #EDE0D4; -fx-font-size: 13px; -fx-font-weight: bold; -fx-background-radius: 20; -fx-padding: 7 18 7 18; -fx-cursor: hand;");
    }

    private void styleTabInactive(Button btn) {
        btn.setStyle("-fx-background-color: #E6CCB2; -fx-text-fill: #7F5539; -fx-font-size: 13px; -fx-background-radius: 20; -fx-padding: 7 18 7 18; -fx-cursor: hand;");
    }

    private void showDescriptionTab(VBox container) {
        container.getChildren().clear();
        Label descLabel = new Label(produit.getDescription() != null ? produit.getDescription() : "Aucune description disponible.");
        descLabel.setStyle("-fx-font-size: 14px; -fx-text-fill: #3B1F0E; -fx-wrap-text: true; -fx-line-spacing: 4;");
        descLabel.setMaxWidth(Double.MAX_VALUE);
