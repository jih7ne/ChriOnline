...
public class ServerUDPTest {
    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            ServerConfig.getLogger().info("✅ All tests passed");

        } catch (AssertionError e) {
            ServerConfig.getLogger().error("❌ Test failed: {}", e.getMessage());
            throw e;
        } finally {
            stopInstances();
        }
    ...
    private static void setup() throws Exception {
        ServerConfig.getLogger().info("--- Setting up test environment ---");

        // Server
        udpNotificationDispatcher = new UDPNotificationDispatcher();
        udpNotificationDispatcher.setNotificationHandler(notification -> {
            ServerConfig.getLogger().info("Server received notification: {}", notification.getMessage());
        ...
        udpListener.setNotificationHandler(notification -> {
            ServerConfig.getLogger().info("Client received notification: {}", notification.getMessage());
        ...
        Thread.sleep(500);
        ServerConfig.getLogger().info("Setup complete — server clients registered: {}",
    ...
    private static void testClientRegisters() throws InterruptedException {
        ServerConfig.getLogger().info("--- testClientRegisters ---");
        ...
        ServerConfig.getLogger().info("PASS — registered clients: {}", udpNotificationDispatcher.getConnectedClientCount());
    }
    ...
    private static void testClientReceivesBroadcast() throws InterruptedException {
        ServerConfig.getLogger().info("--- testClientReceivesBroadcast ---");
        ...
        ServerConfig.getLogger().info("PASS — client received: {}", last.getMessage());
    }
    ...
    private static void testHeartbeatDelivered() throws InterruptedException {
        ServerConfig.getLogger().info("--- testHeartbeatDelivered ---");
        ...
        ServerConfig.getLogger().info("PASS — heartbeats received: {}", countAfter - countBefore);
    }
    ...
    private static void testMultipleClientsReceiveNotification() throws Exception {
        ServerConfig.getLogger().info("--- testMultipleClientsReceiveNotification ---");
        ...
        try {
            secondListener.setNotificationHandler(n -> {
                ServerConfig.getLogger().info("Second client received: {}", n.getMessage());
            ...
            ServerConfig.getLogger().info("PASS — both clients received the broadcast");
    ...
    private static void testServerHandlesIncomingNotification() throws Exception {
        ServerConfig.getLogger().info("--- testServerHandlesIncomingNotification ---");

        CountDownLatch serverReceivedLatch = new CountDownLatch(1);

        udpNotificationDispatcher.setNotificationHandler(notification -> {
            ServerConfig.getLogger().info("Server handler fired for: {}", notification.getId());
        ...
        ServerConfig.getLogger().info("PASS — server handled incoming client notification");
    }
