...
public class CheckoutView extends BorderPane {
    ...
    public CheckoutView(TCPClient tcpClient,
        ...
        VBox expireBox = new VBox(2,
                new Label("Expire") {
                    {
                        setStyle("-fx-text-fill: rgba(255,255,255,0.5); -fx-font-size: 10px;");
                    }
                },
                datePrevLabel);
        ...
        new Thread(() -> {
            try {
                AppRequest req = new AppRequest.Builder()
                        ...
                        .authToken(tcpClient.getAuthToken()) // 🔑 requis pour OwnershipValidator
                        .build();
    ...
    private void handlePaiement() {
        ...
        new Thread(() -> {
            try {
                ...
                paiementParams.put("methodePaiement", "CARTE_BANCAIRE");
                ...
                if (respPaiement == null || !respPaiement.isSuccess()) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        viewManager.showConfirmationEchoueeView(
                                ...
                                errMsg,
                                () -> {
                                    List<PanierProduit> mappedItems = lignes.stream().map(l -> {
                                            PanierProduit pp =
                                                    new PanierProduit();
